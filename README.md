Working link - upi-offline-mesh-production-4af9.up.railway.app
...
### Run on Mac/Linux
=======
#  UPI Offline Mesh Network Demo

A Spring Boot based simulation of an **offline-first UPI payment system** where encrypted payment packets travel across nearby devices using a mesh network until a device with internet connectivity uploads the transaction to the backend.

---

##  Problem Statement

Digital payments depend heavily on internet connectivity.
...
UPI transactions may fail because devices cannot directly reach backend servers.

This project explores an alternative approach where payment requests propagate through nearby devices until a bridge node with internet access settles the transaction.
...
* Gossip-style device communication
* End-to-end encrypted packets
* Bridge node upload simulation
...
```bash
git clone https://github.com/ektakumari22/UPI-Offline-Mesh.git
```
...
Linux / Mac:
>>>>>>> a74df00358e1adce90041e3886d49a5288259ec1
...
Encrypted packet enters mesh network.

### 3. Gossip Round

Devices forward packets to nearby devices.

### 4. Bridge Upload

A device with internet connectivity uploads packets to backend.

### 5. Settlement

Transaction is processed exactly once using idempotency protection.
